...
class cliquenet(nn.Module):
    def __init__(self,num_classes,input_channels=64, list_channels=[36,36,36], list_layer_num=[4,4,4], if_att=False):
        super(cliquenet, self).__init__()
        self.fir_trans = nn.Conv2d(3, input_channels, kernel_size=3, stride=1, padding=1, bias=False)
...
#        self.fir_pool = nn.MaxPool2d(kernel_size=2, stride=1, padding=1)
        ...
        self.fc = nn.Linear(in_features=sum(self.list_gb_channel) // 2, out_features=num_classes)

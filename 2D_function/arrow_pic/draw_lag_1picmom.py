...
def fn(x_start, index=FN_INDEX):
    ...
    fn_list = [
        ...
        -(np.cos(x) + 1) * (np.cos(2 * y) + 1),  # formula 3-------------

        (1 - x) * (1 - x) + 100 * (y - x * x) * (y - x * x),  # 5

        (1 + (x + y + 1) * (x + y + 1) * (19 - 14 * x + 3 * x * x - 14 * y + 6 * x * y + 3 * y * y)) * (
                    30 + (2 * x - 3 * y) * (2 * x - 3 * y) * (
                        18 - 32 * x + 12 * x * x + 48 * y - 36 * x * y + 27 * y * y)),
        0.26 * (x * x + y * y) - 0.48 * x * y,  # 11
...
def g(x0, index=FN_INDEX):
    ...
    grad_list = [
        ...
        np.array([np.sin(x) * (np.cos(2 * y) + 1), 2 * np.sin(2 * y) * (np.cos(x) + 1)]),  # formula 3 's grad--------
...
ax2.set_yticks([-2,-1.5,-1,-0.5,0,0.68,1,1.5,2])
...
plt.axvline(21,ls="--",linewidth=1,color="black")
...
plt.axhline(0.68,ls="--",linewidth=1,color="black")
